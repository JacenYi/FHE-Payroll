...
const decryptValue = useCallback(async (handle, contractAddress, account, signer) => {
  ...
  const publicDecrypt = useCallback(async (handles) => {
    try {
      console.log('[FHE] publicDecrypt 参数:', { handles });
      
      if (!handles || !Array.isArray(handles) || handles.length === 0) {
        throw new Error('handles 必须是非空数组');
      }
      
      const relayer = await initRelayer();
      
      const results = await relayer.publicDecrypt(handles);
      
      console.log('[FHE] publicDecrypt 结果:', results);
      
      return {
        success: true,
        clearValues: results.clearValues || results.values,
        abiEncodedClearValues: results.abiEncodedClearValues,
        decryptionProof: results.decryptionProof,
        error: null,
      };
    } catch (err) {
      console.error('[FHE] publicDecrypt 失败:', err);
      return {
        success: false,
        clearValues: null,
        abiEncodedClearValues: null,
        decryptionProof: null,
        error: err.message || 'publicDecrypt 失败',
      };
    }
  }, [initRelayer]);
